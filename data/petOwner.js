...
async function addPetOwner(profilePicture,fullName,emailAddress,password,phoneNumber,zipCode,biography,dateOfBirth,favoritedPets,websiteFeedbackGiven,shelterReviewsGiven,reportedPosts,donatedItems,isVolunteerCandidate){
    ...
    if (!fullName || !emailAddress || !password || !phoneNumber || !zipCode || !biography){
...
async function getPetOwnerByUserEmail(petOwnerEmail){
    ...
    const petOwnerDetails = await petOwnerCollection.findOne({emailAddress:petOwnerEmail});
...
async function updatePetOwner(updatedData){
    ...
    if(updatedData.fullName){
        if(updatedData.fullName.hasOwnProperty('firstName') && updatedData.fullName.firstName.trim() != ""){
         ...
         if(updatedData.fullName.hasOwnProperty('lastName') && updatedData.fullName.lastName.trim() != ""){
    ...
    const updateInfo = await petOwnerCollection.updateOne({ _id: existingUserData._id }, { $set: modifiedData});
    if (updateInfo.matchedCount === 0 && updateInfo.modifiedCount === 0)  throw 'Could not update user';
