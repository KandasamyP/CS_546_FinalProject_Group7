  ...
  try {
    ...
    for (let i = 0; i < shelter.reviews.length; ++i) {
      if (shelter.reviews[i].reviewer) {
        const petOwnerInfo = await petOwnerData.getPetOwnerById(
          shelter.reviews[i].reviewer
        );
...
router.post("/addReviews/:id", async (req, res) => {
  try {
    ...
    if (shelter.reviews.length > 0) {
      ...
      const petOwnerInfo = await petOwnerData.updateShelterReviewsGiven(
        shelter.reviews[shelter.reviews.length - 1].reviewer,
        shelter.reviews[shelter.reviews.length - 1]._id
      );
    } else {
      reviewDetail = {
        avgReviews: 0,
